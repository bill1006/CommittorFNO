function sol = committor_2Well_in_1D(beta)
close all
...
x_eval =  -1:.01:1 ;
% -1:0.01:1; 
% points for solution evaluation
...
L = D2 - beta*(Vder(x(2:N))*ones(1,N-1)).*D % the differential operator
f = beta*0.5*Vder(x(2:N)) % -L applied to the boundary function ubdry = 0.5*(1+x) computed exactly.
